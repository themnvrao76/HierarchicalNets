...
class MaxVit(nn.Module):
    def __init__(self,Pretrained_Model):
        ...
        self.lin1=nn.Linear(in_features=256,out_features=6,bias=True)
        self.lin2=nn.Linear(in_features=256,out_features=20,bias=True)
        self.norm_o1=LayerNorm2d((256,), eps=1e-06)
        self.norm_o2=LayerNorm2d((256,), eps=1e-06)
        self.steam2=Pretrained_Model.stages[2]
...
model=MaxVit(Pretrained_Model).to(device)
...
checkpoint = torch.load("/home/22mces01/MaxVit_224/MaxVIT_Herarchical_Transformer/Hierarchical_CoatNet_Best_BESTMODEL.pth.tar")
...
def EvalMode():
  ...
  with torch.no_grad():
    for data in tqdm(testset,desc="Eval Loop",position=1):
        ...
        top51.append((torch.topk(Out1.data, 5).indices==orig1).sum().item())
